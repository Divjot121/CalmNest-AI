...
import { Sparkles, Leaf, Shield, Lock, Loader2, CheckCircle2, AlertTriangle } from 'lucide-react';
import { useAuthStore } from '@/store/useAuthStore';
import { triggerGentleSanctuaryCelebration } from '@/components/SanctuaryConfetti';

export default function SignupPage() {
  const router = useRouter();
  const { user, signup, loginWithGoogle } = useAuthStore();
  ...
  return (
    <main className="min-h-screen bg-[#FAF9F6] dark:bg-[#16181D] flex flex-col justify-center items-center p-4 transition-colors duration-300">
      <motion.div
        ...
        className="w-full max-w-md bg-white dark:bg-[#1E2128] border border-slate-200/70 dark:border-[#2B2F38] rounded-3xl p-6 sm:p-8 shadow-xs space-y-5"
      >
        {/* Logo */}
        <div className="flex flex-col items-center text-center space-y-2">
          ...
          <h1 className="text-xl font-medium text-slate-900 dark:text-slate-100">Create Permanent Account</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">Lock in your streak and backup your journey</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Choose Password</label>
            <input
              type="password"
              value={password}
        ...
        <div className="relative flex py-1 items-center">
          ...
          <span className="flex-shrink mx-3 text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider font-medium">Or join with</span>
          <div className="flex-grow border-t border-slate-200 dark:border-[#2B2F38]"></div>
        </div>
        ...
        <div className="text-center pt-2">
          <p className="text-xs text-slate-500 dark:text-slate-400">
            ...
            <Link href="/login" className="text-primary dark:text-[#A1C2D4] hover:underline font-semibold">
              Sign In
            </Link>
          </p>
        </div>
